...
function Footer() {
  const tagline = useContent(
    "footer.tagline",
    "Empowering job seekers and professionals with AI-driven learning tools for skill mastery, interview preparation, and career growth.",
  );
  const contactEmail = useContent("footer.contact.email", "support@nexo.ai");
    ...
    <footer className="bg-secondary text-secondary-foreground mt-auto">
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="border-t border-secondary-foreground/10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-secondary-foreground/40">
          <p>© {new Date().getFullYear()} Nexo Platform. All rights reserved.</p>
          <p>AI-Powered Career &amp; Development</p>
        </div>
      </div>
    </footer>
  );
